...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        """
        Args:
            rnn_type: The type of RNN to use as decoder in the module.
                Must be a class inheriting from torch.nn.RNNBase
                (such as torch.nn.LSTM for example).
            input_size: The number of expected features in the input of the
                module.
            hidden_size: The number of features in the hidden state of the RNN
                used as encoder by the module.
            num_layers: The number of recurrent layers in the decoder of the
                module. Defaults to 1.
            bias: If False, the decoder does not use bias weights b_ih and
                b_hh. Defaults to True.
            dropout: If non-zero, introduces a dropout layer on the outputs
                of each layer of the decoder except the last one, with dropout
                probability equal to 'dropout'. Defaults to 0.0.
            bidirectional: If True, the decoder of the module is bidirectional.
                Defaults to False.
        """
        ...
        self._decoder = rnn_type(input_size,
                                 hidden_size,
                                 num_layers=num_layers,
                                 bias=bias,
                                 batch_first=True,
        ...
        self._vocab_proj = nn.Linear(self.hidden_size, self.vocab_size)
        
    def forward(self, sequences_batch, init_state=None, isTrain=False):
        """
        Args:
            sequences_batch: A batch of variable length sequences of vectors.
                The batch is assumed to be of size
        ...
        Returns:
            reordered_outputs: The projection to vocabulary size of the outputs (hidden states) of the decoder for
                the sequences in the input batch, in the same order.
        """        
#        sorted_batch, sorted_lengths, _, restoration_idx =\
#            sort_by_seq_lens(sequences_batch, sequences_lengths)
...
#                                                         batch_first=True)
        ...
        outputs, ht = self._decoder(sequences_batch, init_state)

#        outputs, _ = nn.utils.rnn.pad_packed_sequence(outputs,
#                                                      batch_first=True)
#        reordered_outputs = outputs.index_select(0, restoration_idx)
        
        projected_outputs = self._vocab_proj(outputs)

        return projected_outputs, ht            
